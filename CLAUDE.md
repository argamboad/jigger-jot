...
8. **Platform wins — on the foundation.** App docs never override platform mechanics; when they
   ...
   own** (JJ-039): every screen in the RCL, the inherited ones included, is downstream property and is
   restyled here without going upstream. **The backend may be extended, never re-founded** — new
...
| `docs/PLAN.md` | **Read first, every session** — the three gates, the slice ritual, the editing rules, where things stand, and the sequenced backlog (every feature flow shipped; next is the `BACKBAR` restyle ladder, 1 → 8) |
