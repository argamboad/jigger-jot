...
4. **Every volume is stored in ounces, on the quarter marks of a jigger, and read in the viewer's
   system** — ounces, or millilitres at the bar's **30 ml to the ounce** in 2.5 ml steps (JJ-041,
   amending JJ-007). Parts become their share of a 3 oz drink, a glass or wineglass is 2 oz, a
   liqueur glass 1 oz; teaspoons, tablespoons and neutral units (dash, barspoon, piece) stay as
   written. **`BarMeasure` in Core** is the one table: the seeder and the authoring handler write
   through it, **`AmountDisplay`** reads through it, server-side, so a second front end inherits the
   rules. Two preferences, imperial the default — a null one reads as imperial, and there is **no
   "as written"** any more. The seed JSON keeps the books' amounts; the table keeps ounces.
...
8. **Platform wins — on the foundation.** App docs never override platform mechanics; when they
   disagree, defer to the platform and log a `JJ-` decision (JJ-026, JJ-028). **The UI is the app's
   own, all pages** (JJ-039): every screen in the RCL, the inherited ones included, is downstream
   property and is restyled here without going upstream. **The backend may be extended, never re-founded** — new
   endpoints and features are the app's to add; auth, tenancy, the outbox, billing, the RLS backstop
   and R1–R76 are a red light.
...
- **Epic keys** for stories and scopes: `INGREDIENT`, `INV` (inventory), `CKTL` (cocktail catalog),
  ...
  `MARGA` (the UI character), `SHELL` (header, responsive nav, boot state) and `BACKBAR` (the
  restyle to design direction 1A — presentation only, never behaviour). Feature
...
| `docs/PLAN.md` | **Read first, every session** — the three gates, the slice ritual, the editing rules, where things stand, and the sequenced backlog (every feature flow shipped; the `BACKBAR` restyle ladder built 1 → 9 on `feat/backbar`; what follows is a decision, not a queue) |
...
| `docs/QA_TEST_PLAN.md` | Manual QA plan — step-by-step tests across web + all four native platforms (235 cases: smoke + regression + §10d–§10i the app's own flows + §14a v3-audit adversarial/tenant-isolation + §14c pre-launch gates + §13c native release checklist). **§10d–§10i are JiggerJot's own flows** — the shelf, the catalog and filters, makeable/one-away, forking and authoring, the onboarding wizard, and Marga plus the chrome. They were added in one pass after nineteen app slices had shipped against a plan that covered the platform only; **the slice ritual in `docs/PLAN.md` now carries a step so it cannot drift again** |
...
| `docs/design/` | **Design handoffs, committed because the slices cite their pages.** `2026-09-backbar-handoff.pdf` — direction 1A "Back bar", 19 pages: tokens (p. 01), components (p. 02), nine screens in both themes (p. 03–17), the implementation order and the "do not change" list (p. 18). The earlier 2026-09-10 proposal lived in the Claude Design project only |
| `docs/stories/backbar.md` | epic `BACKBAR` ✅ COMPLETE (built 2026-09-13, nine slices on one branch) — **BACKBAR-1 ✅ the foundation** (both token sets, Instrument Serif self-hosted as two subsets with its OFL beside it and reached only through `.font-display`, the primitives as CSS on Bootstrap's classes, `MargaSays` `Tone` with `Size`/`Compact` intact; three repo gates hold the face self-hosted, licensed and never bold) · **BACKBAR-2 ✅ the chrome** (header and tab bar on the surface with a hairline in both themes, `navbar-dark` gone, destinations in the ink and the account cluster muted on one theme-aware button variant, a 2px copper indicator, `icon_dark.svg` for the dark surface swapped by `app.css`; the shell journey's three literal-colour lines rewritten to the two relations they guard) · **BACKBAR-3 ✅ Home + Shelf** (Home in two columns, rules not cards, the unlock panel the only box, the count at 128px in the serif, her scene on an empty shelf and never a zero, a brass loading bar instead of a spinner; the shelf's sixteen cards become sections on a hairline with the anchors intact, the bottle Marga names outlined in the subtle copper and never ticked, the add form a panel in place, the payoff count in the serif — as a `<span>`, because a `<strong>` would ask the one-weight face for bold) · **BACKBAR-4 ✅ Cocktails + Detail** (the two switches become three RADIOS with one new id, `cocktail-all`, driven by label through `SetCatalogFilterAsync` in `E2ETestBase` — the one slice that touches a journey's mechanics; only the selected chip carries the response's own total, no new fetch; rows keep their list-group classes as hairlines with Marga inline; the recipe's amounts lead in the serif in a 104px column, two marks only, optional as a dash and plain text, provenance in the facet line, a fixed fork bar on a phone, `@media print`, not-found on the empty-state layout; `MargaSays.Size` became nullable so a tone can be drawn at 72) · **BACKBAR-5 ✅ Login + Welcome** (one `MargaSplit` component — her scene as half the screen with the line on it, night ground in BOTH themes, the wordmark over it, the working side beside; Login and Welcome are her two full appearances, Join and the auth error page reuse it with the error heading as the line and no label; the wizard gains a 3px brass progress rule and its step 2 renders the shelf's own sections, whose rules moved to `app.css`; the code step is one 24px spaced field; every id intact, nothing boxed) · **BACKBAR-6 ✅ Write** (two columns, the amount in the display face beside its unit, role and required a visible sub-line — no popover, F4 — the two client checks attached to the name field and the lines with `new-error` kept above Save for the server, a fixed Save bar on a phone; page 15's "fork = the same form pre-filled" is AUTHORING-2, not FORK-1, and stays outstanding) · **BACKBAR-7 ✅ Settings + Household** (five cards to two columns of labelled hairline-row groups, six to four; `ThemeSwitcher`/`UnitSwitcher` gain `Segmented` — radios on their own ids sharing the select's handler, which also ends a page carrying `theme-switcher` twice; Unlink, Link, Make admin, Remove, Regenerate, Revoke and Download as text links, the destructive ones red behind the same confirms; a ··· row menu on a phone that toggles visibility only; Owner the only copper badge; the name in the serif for non-owners; the bell's unread dot brass; the two cards restyled inside with the same parameters; one journey's two dropdown selects on the unit switcher become label clicks) · **BACKBAR-8 ✅ Billing + Admin + every small screen** (the pages the handoff never drew, by analogy: Billing as two groups with the plan in the serif and the status pill in the status colours; the console's broadcast as the one panel, tenants and members as hairline rows, text-link actions, every status one `.notice`; not-found as a `NotFoundView` on the empty-state layout, the callback's spinner a shared `.loading-bar`, the impersonation banner a hairline strip, the error bar in the error colours; no admin write added) · **BACKBAR-9 ✅ the sweep** (ninety-six frames — sixteen screens × three widths × two themes — rendered against mocked responses and read one by one; three findings, each fixed test-first: the bell's dropdown was still a card, every primary button that STARTS disabled was Bootstrap blue because the copper rule never set the disabled tokens, Join's headings were still bold sans; the boot arc joins the reduced-motion block; three repo gates hold the definition of done — no `.card` outside the unlock panel, every transition and animation named again under `prefers-reduced-motion` in the SAME sheet since scoped CSS cannot be reached from `app.css`, and the primary button's disabled tokens set). Planned 2026-09-13 — **the restyle**, and nothing but: dark primary + light counterpart, one self-hosted display serif, hairline surfaces, the chrome off copper, Marga at 96px where she speaks and full-bleed on Login and Welcome. **Reviewed and decided before anything was built** (JJ-036 → JJ-039): the handoff's own "do not change" list is binding; two tests pin what it changes (the shell journey's literal chrome colours, the makeable journey's nine `Check`/`Uncheck` calls on switches that become hidden inputs) so the catalog filter becomes three RADIOS with one new id and the colour assertions are rewritten to the hierarchy they guard; two of its id lists are invented (`write-*`, `household-*`) so the razor is the contract; no popover on Write rows; **the UI is the app's own, all pages, no exceptions**, so Settings and Household are restyled in full, Billing and Admin too though the handoff never drew them, nothing is sent upstream, and the epic touches no backend. Nine slices in the document's order — foundation (tokens + serif + primitives + `MargaSays` `Tone`, three commits one PR) → chrome → Home + Shelf → Cocktails + Detail → Login + Welcome → Write → Settings + Household → Billing + Admin + every small screen → sweep. Every slice runs both themes at 390/768/1440 before its commit: 54 frames and no test looks at any of them |
