...
| `docs/stories/almost.md` | epic `ALMOST` ✅ COMPLETE for MVP — ALMOST-1 `GET /api/cocktails?almost=true` + a second toggle on the catalog screen, exclusive with the makeable one. Exactly ONE required line unsatisfied **after substitutions** (FEATURES §10, JJ-019), and every row names that bottle in `missingIngredient` — the name is the feature, since "one short" alone is just a list of drinks you cannot make. Same predicate as MAKE-1's, counted rather than negated, so the two lists are adjacent and can never overlap; asking for both filters returns an empty page rather than one flag winning. Rows still carry the substitutions in play (FEATURES §9) |
...
| `docs/stories/marga.md` | epic `MARGA` 📋 PLANNED — the UI character. **Not an assistant**: a drawn bartender who says FIXED resource strings with real data in the placeholders; no model, no generated text. MARGA-1 her asset + one shared component + three lines over existing data (proposal screens 2, 3, 8) · MARGA-2 the home screen, closing the `Home.razor` TODO (screen 1) · MARGA-3 the two empty states (screens 6, 7), ⚠️ blocked on what the best first bottle is for an empty shelf |
| `docs/stories/shell.md` | epic `SHELL` 📋 PLANNED — the chrome. SHELL-1 bottom tab bar below `lg` with the account cluster staying on top (⚠️ must keep the `nav-shelf` / `nav-cocktails` test ids) · SHELL-2 the boot state, which lands in **two** `index.html` files per `NATIVE_PARITY.md` |
