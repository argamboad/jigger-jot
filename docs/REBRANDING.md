...
- **The Android system bars** — `src/Maui/Platforms/Android/Resources/values/colors.xml` (the theme's
  ...
  `src/Maui/Platforms/Android/AndroidSystemBarTheme.cs`, whose `Light`/`Dark` must equal app.css's
  `--surface` and `LightGround`/`DarkGround` its `--app-bg`, in each theme: the status bar is drawn by the
  OS above the header (or above the bare page on the sign-in screens), and the page paints it through
  `SystemBarThemeSync`. The template shipped a sage green in both files that no rebrand touched,
  so every downstream Android app wore it (found in JiggerJot and y-el-vuelto, 2026-09-16).
