...
- **`ApiBaseUrl` becomes optional** in `src/Web`: default to `builder.HostEnvironment.BaseAddress`
  ...
  `https://localhost:7360`; the deployed bundle ships without it. (The e2e CI job's appsettings
...
actually deployed — instead of failing after the 12½-minute wait (found on a downstream app's develop).
