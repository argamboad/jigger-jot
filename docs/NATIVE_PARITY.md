...
| **JS interop / vendored scripts** | ✅ | `src/Maui/wwwroot/index.html` includes the same `_content/JiggerJot.Shared.Ui/js/qrcode-generator.min.js` + `mfa-qr.js` as web — the MFA QR renders natively. (Keep the two `index.html` files in sync — noted as a maintainer rule below.) |
...
- **The two `index.html` hosts must stay in sync** (`src/Web/wwwroot/index.html` ↔
  ...
  `_content/JiggerJot.Shared.Ui/css/app.css`; only the bootstrap vendored copies and the script
