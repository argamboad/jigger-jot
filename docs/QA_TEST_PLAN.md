...
>
> Brand in examples is "JiggerJot"; substitute your app's brand if rebranded.
...
dotnet run --project src/Api --launch-profile https    # binds https:7360 (web/desktop) AND http:5438 (android)
```

- API health/liveness check: `curl -k https://localhost:7360/health` → **200** (`Healthy`);
  `curl -k https://localhost:7360/health/ready` → **200** when the database is reachable (503 if not).
  *(The older `curl -k -X POST https://localhost:7360/api/auth/refresh` → **401** reachability check
  still works.)*
- **Mailpit UI: <http://localhost:8027>** — this is the dev mail trap. Every magic link, OTP code,
  ...
  > - commenting out the `Email__Smtp__*` lines in `.env` (unset → defaults to Mailpit `localhost:1027`), **or**
  ...
  >     --Email:Smtp:Host=localhost --Email:Smtp:Port=1027 --Email:Smtp:Username= --Email:Smtp:Password=
  >   ```
  ...
  > a request error.
- Web app: `dotnet run --project src/Web --launch-profile https` → **<https://localhost:7208>**.
...
- API must be running on `https://localhost:7360` (the desktop client's base URL).
...
- **`adb reverse tcp:5438 tcp:5438`** — **re-run every time the device/emulator restarts** (it does
  not persist). Verify with `adb reverse --list`.
- API started with the **https** profile (binds the cleartext `:5438` leg the device uses).
- Provider redirect URIs registered: `http://localhost:5438/signin-google` and
  `http://localhost:5438/signin-microsoft`.
...
```
**Walkthrough**
1. Open <https://localhost:7208> → you're redirected to `/login`.
...
4. Open Mailpit (<http://localhost:8027>); open the newest mail; copy the 6-digit code.
...
```
**Walkthrough**
1. `curl -k https://localhost:7360/health` → **200**, body `Healthy` (liveness — process up).
2. `curl -k https://localhost:7360/health/ready` → **200** (readiness — Postgres reachable).
...
**the only boxed thing on the page**. At 390 the same in one column. With nothing ticked: her scene
...
> Prereq every run: **`adb reverse tcp:5438 tcp:5438`** + API on the https profile. See
...
```
**Walkthrough**
1. Confirm `adb reverse --list` shows `tcp:5438`. Launch the app.
...
Then the in-app browser returns to the app via the jiggerjot:// scheme, signed in
```
**Walkthrough**
1. Tap **Continue with Google**. **Expected:** a browser tab opens to Google consent.
2. Approve. **Expected:** the tab returns control to the app (`jiggerjot://auth` intent), now signed
   in. (Repeat **Microsoft** if its `:5438` redirect is registered.)
...
2. With the tab in the foreground, kill the app process:
   `adb shell am kill com.jiggerjot.app` (works because the app is backgrounded behind the
   browser; *Don't keep activities* + memory pressure reproduces it the organic way).
3. Approve consent in the still-open tab. **Expected:** the `jiggerjot://auth` redirect cold-starts
...
> iOS **simulator** shares the host network, so `https://localhost:7360` works; a **physical device**
...
> `IOAuthInitiator` registered). OAuth is wired via `ASWebAuthenticationSession` + the `jiggerjot`
...
Google consent; approving returns to the app via the `jiggerjot://auth` scheme, signed in. Also
...
`jiggerjot://` scheme; arbitrary redirect targets are rejected. This is unit-tested
...
> those are flagged in the title. Base URL `https://localhost:7360` (use `-k` for the dev cert) locally,
...
2. **Expected (post-remediation):** base URL is the configured **HTTPS** API; a missing base URL **fails
   the build**; no `http://localhost:5438` and no cleartext-permitting network config is shipped.
...
> steps is `https://localhost:7360` (use `-k` for the dev cert).
...
1. With both flags **unset/false**, restart the API and call
   `curl -k https://localhost:7360/api/public/openapi.json` and `.../api/apikeys` (with a JWT).
...
```
**Walkthrough**
1. `curl -k -X POST https://localhost:7360/api/apikeys -H "Authorization: Bearer <JWT>" -H "Content-Type: application/json" -d '{"name":"qa","scopes":["read"]}'`
...
```
**Walkthrough**
1. `curl -k https://localhost:7360/api/public/whoami -H "X-Api-Key: pk_…"` → **200**, body shows my
   `tenant_id`, the key name, and `["read"]`.
2. `curl -k -X POST https://localhost:7360/api/public/echo -H "X-Api-Key: pk_…" -d '{"message":"hi"}'`
...
- **Updated 2026-07-06** — **first Apple run of §13b** (maintainer's MacBook Air M1, iOS 26.5 simulator +
  ...
  **QA-IOS-02 PASS**, **QA-IOS-04 PASS** (first-ever exercise of the `ASWebAuthenticationSession` →
  `jiggerjot://auth` path; also verified on iPhone 17 Pro Max + iPad Air 11" simulators), **QA-MAC-01
