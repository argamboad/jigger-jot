...
|---------|-------|-----------------|---------|
| `db` | `postgres:17` | `${DB_PORT:-5432}` (committed `.env.example` sets **5435**) | PostgreSQL — matches production DB engine |
| `mail` | `axllent/mailpit:latest` | SMTP `${MAIL_SMTP_PORT:-1027}`, UI `${MAIL_UI_PORT:-8027}` | Local SMTP trap for passwordless + invitation emails |
