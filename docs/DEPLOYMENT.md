...
1. Push your branch; in Render choose **New + → Blueprint** and point it at the repo. It reads
   `render.yaml` (service `jiggerjot-staging`, Docker runtime, free plan, health check `/health/ready`).
...
3. Deploy. When it's live, copy the public URL (e.g. `https://jiggerjot-staging.onrender.com`) and set
...
```powershell
dotnet publish src/Maui/JiggerJot.Maui.csproj -f net10.0-android -c Release `
...
```powershell
dotnet publish src/Maui/JiggerJot.Maui.csproj -f net10.0-windows10.0.19041.0 -c Release `
  -p:ApiBaseUrl=https://<your-host> -p:WindowsPackageType=None -o out/windows
```

Run `out/windows/JiggerJot.Maui.exe` from that folder (pin a shortcut). SmartScreen warns once because it is
