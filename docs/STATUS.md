...
4. **Android:** start the emulator, `adb reverse tcp:5438 tcp:5438`
...
4. Start the API: `dotnet run --project src/Api --launch-profile https`. Verify
   <https://localhost:7360/health> returns Healthy; Mailpit UI at <http://localhost:8027>.
...
3. Build & run:
   ...
   ```
   The simulator shares the host network, so `https://localhost:7360` reaches the API directly —
...
| App can't reach the API | API must be on the **https** profile (port 7360); check `/health` in Safari on the Mac |
