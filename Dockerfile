...
COPY src/Core/JiggerJot.Core.csproj src/Core/packages.lock.json src/Core/
COPY src/Infrastructure/JiggerJot.Infrastructure.csproj src/Infrastructure/packages.lock.json src/Infrastructure/
COPY src/Api/JiggerJot.Api.csproj src/Api/packages.lock.json src/Api/
COPY src/Shared.Ui/JiggerJot.Shared.Ui.csproj src/Shared.Ui/packages.lock.json src/Shared.Ui/
COPY src/Web/JiggerJot.Web.csproj src/Web/packages.lock.json src/Web/
...
RUN dotnet restore src/Api/JiggerJot.Api.csproj --locked-mode \
 && dotnet restore src/Web/JiggerJot.Web.csproj
...
RUN dotnet publish src/Web/JiggerJot.Web.csproj -c Release --no-restore -o /publish/web \
 && dotnet publish src/Api/JiggerJot.Api.csproj -c Release --no-restore -o /publish/api \
...
# file fails with "Access to the path '/app/storage' is denied" (seen on a sibling app's staging).
...
ENTRYPOINT ["sh", "-c", "exec dotnet JiggerJot.Api.dll --urls http://0.0.0.0:${PORT:-8080}"]
